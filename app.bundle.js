...
const createManager = () => {
    ...
    let _projects = [];
    let _todos = createProject("Todos");
...
const TodoList = (manager) => {
    ...
    let todosProject = manager.getTodos();
    ...
    sidebar.addEventListener("click", (event) => {
        ...
        if(target.matches(".all-todos")){ // all todo button
            ...
            const todosProjectTodos = todosProject.getTodos();
            todosProjectTodos.forEach(todo => {
                if(todo.getProject() !== "Todos" ){
                    todosProject.removeTodo(todo)
                }
            });
            //add project todos
    ...
    board.addEventListener("click", (event) => {
        ...
        if(target.matches(".project-details > .title-holder >.rename")){ //rename button
            ...
            if(currentProject.getTitle() === "Todos"){
            ...
            currentTodo.setProject(currentProject.getTitle());
...
const managerIntoJSON = (manager) => {
    ...
    const todosProject = manager.getTodos();
    const todosProperties = getProjectProperties(todosProject);

    const projects = {

        projects: projectProperties,
        todos: todosProperties
...
const jsonIntoManager = (managerJSON) => {
    ...
    const projects = parseManager.projects;
    projects.forEach(project => manager.addProject(parseIntoProject(project)));

    const todosProject = parseManager.todos;
    const todosManager = manager.getTodos();
    const todos = todosProject.todos;
    todos.forEach(todo => todosManager.addTodo(parseIntoTodo(todo)));
